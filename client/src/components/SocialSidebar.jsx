...
const SocialSidebar = () => {
  return (
    <div className="fixed left-3 sm:left-[20px] top-1/2 -translate-y-1/2 z-40 flex flex-col items-center">
      {/* Top Divider Line */}
      <div className="w-[1.5px] h-6 sm:h-10 bg-brandNavy/30" />

      {/* Social Links List */}
      <div className="flex flex-col gap-2 sm:gap-4 my-2 sm:my-4">
        {socialLinks.map((link) => {
          ...
          return (
            <a
              ...
              className={`group relative flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white border border-gray-200 text-[#0A0F1E] shadow-md transition-all duration-300 hover:text-white hover:scale-110 ${link.colorClass}`}
            >
              <Icon className="w-4 h-4 sm:w-5 h-5 transition-transform duration-300" />
